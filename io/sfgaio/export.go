...
// to be zipped or not
func (s *sfgaio) Export(outFile string, isZip bool) error {
	var err error
	err = s.dumpSQL(outFile + ".sql")
	if err != nil {
		return err
	}
	err = s.dumpBinary(outFile + ".sqlite")
	if err != nil {
		return err
	}
...
func (s *sfgaio) dumpBinary(outFile string) error {
	var err error
	cmd := exec.Command("sqlite3", s.dbPath, ".backup "+outFile)
...
func (d *sfgaio) dumpSQL(outFile string) error {
	cmd := exec.Command("sqlite3", d.dbPath, ".dump")
	...
	if err = cmd.Start(); err != nil {
		return &sfga.ErrFileCopy{Src: d.dbPath, Dst: outFile, Err: err}
	}
